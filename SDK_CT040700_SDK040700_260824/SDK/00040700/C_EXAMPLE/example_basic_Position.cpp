...
int main(int argc, char** argv)
{
    ...
    printf("Press any key to let arm1 move to target position "
           "{%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf}\n",
           pos1[0], pos1[1], pos1[2], pos1[3], pos1[4], pos1[5], pos1[6]);
    getchar();

    if (FX_L1_Runtime_SetJointPosCmd(1, FX_OBJ_ARM1, pos1) != FUNC_RET_SUCCESS)
    {
    ...
    if (FX_L1_Runtime_SetJointPosCmd(1, FX_OBJ_ARM1, pos2) != FUNC_RET_SUCCESS)
    {
        printf("Failed to set arm1's target position\n");
        goto WAIT_EXIT;
    }
    ...
    printf("Arm1 is in STATE_IDLE state now\n");
