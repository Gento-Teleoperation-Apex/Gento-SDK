...
LoadIdenErrCode OnCalLoadDynamic_7_MarvinSRS(IdenCalParaCfg *CalParaCfg, LoadDynamic* loadDyn, const char *userPath)
{
	...
	FX_DOUBLE l4 = CalParaCfg->l4 / 1000.0;
	FX_DOUBLE l7 = CalParaCfg->l7 / 1000.0;
	FX_DOUBLE tor4, tor6;
...
LoadIdenErrCode OnCalLoadTor_7_MarvinCCS(LoadTor* tor, FX_INT32 isNoLoad, const char *userPath)
{
	...
	string strIdenCfgName(strUserPath+"/CfgFile/LoadIdenCfg.txt");
	ifstream ifsConFile(strIdenCfgName, ios::in);
	if (!ifsConFile.is_open()) {
...
LoadIdenErrCode OnCalLoadDynamic_7_MarvinCCS(IdenCalParaCfg *CalParaCfg, LoadDynamic* loadDyn, const char *userPath)
{
	FX_LOG_INFO("Calculating load data...");
	LoadTor loadTor;
	memset(&loadTor, 0, sizeof(loadTor));
	...
	FX_DOUBLE l7 = CalParaCfg->l7 / 1000.0;
	FX_DOUBLE tor4, tor6;
	tor4 = (loadTor.torLoad[0] - loadTor.torNoLoad[0]);
	tor6 = (loadTor.torLoad[1] - loadTor.torNoLoad[1]);
...
LoadIdenErrCode OnCalLoadDyn(IdenCalParaCfg *CalParaCfg, LoadDynamicPara *DynPara, FX_INT32 RobotType,  const FX_CHAR* UserPath)
{
	DynPara->m =    0;
	DynPara->r[0] = 0;
	DynPara->r[1] = 0;
	...
	if (1 == RobotType) {
		...
		retCode = OnCalLoadDynamic_7_MarvinCCS(CalParaCfg, &dy, UserPath);
		if (retCode != LOAD_IDEN_NoErr) {
			return retCode;
		}
		...
		retCode = OnCalLoadDynamic_7_MarvinSRS(CalParaCfg, &dy, UserPath);
		if (retCode != LOAD_IDEN_NoErr) {
			return retCode;
		}
