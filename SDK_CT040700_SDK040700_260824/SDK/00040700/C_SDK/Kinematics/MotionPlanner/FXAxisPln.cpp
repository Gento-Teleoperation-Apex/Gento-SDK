...
FX_BOOL CAxisPln::OnMovL_KeepJ_CutA(FX_INT32 RobotSerial, Vect7 startjoints, Vect7 stopjoints, FX_DOUBLE vel, FX_DOUBLE acc, CPointSet *ret_pset)
{
	...
	/// ABC
	double cosangle = q_start[0] * q_stop[0] + q_start[1] * q_stop[1] +
	...
	if (0)
	{
		char *path = (char *)"../joint.txt";
		ret_pset->OnSave(path);
	}
...
FX_BOOL CAxisPln::OnMovL_ZSP(FX_INT32 RobotSerial, Vect7 ref_joints, Vect6 start_pos, Vect6 end_pos, FX_DOUBLE vel, FX_DOUBLE acc, FX_DOUBLE jerk, FX_INT32 ZSP_type, Vect6 ZSP_para, FX_DOUBLE Allow_Range, FX_INT32 Point_State)
{
	...
	if (com_axis_len_ < 0.1)
	{
		FX_LOG_ERRO("The input points are too close. Please adjust the input points.");
		return FX_FALSE;
	}
	...
	FX_LOG_DEBG("Motion Planning IK Solve over and start to deal with overlap");
	...
	else
	{
		if (Overlap_Tag && tmp_out_num > Overlap_Num)
		{
			...
			{
				Overlap_Num = num1 - 5;
				FX_LOG_DEBG("last path points = %d", num1);
				if (Overlap_Num <= 0)
				{
					Overlap_Num = 0;
					FX_LOG_WARN("Number of points cannot do overlap.");
				}
			}
			for (i = 0; i <= Overlap_Num; i++)
			{
				...
				if (p1 == NULL)
				{
					FX_LOG_ERRO("Last path points isn't enough");
				}
...
FX_BOOL CAxisJointPln::OnMovJoint(FX_INT32 RobotSerial, Vect7 start_joint, Vect7 end_joint, FX_DOUBLE vel_ratio, FX_DOUBLE acc_ratio, CPointSet *ret_pset)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		{
			if (retp[j] < m_PosNeg[j] - 0.01 || retp[j] > m_PosPos[j] + 0.01)
			{
				m_LastError = FX_PLANNER_JOINT_LIMIT;
...
FX_INT32 CAxisJointPln::OnPln(Vect8 startp, Vect8 stopp, FX_DOUBLE vel_ratio, FX_DOUBLE acc_ratio)
{
	if (m_dof <= 0 || m_dof > 8)
	{
		FX_LOG_ERRO("OnPln: Invalid DOF value: %d", m_dof);
		return -1;
	}
	{
		FX_INT32 i;
		for (i = 0; i < m_dof; i++)
		{
			if (startp[i] < m_PosNeg[i] - 0.01 || startp[i] > m_PosPos[i] + 0.01)
			{
				FX_LOG_ERRO("OnPln: Joint %d's angle is %f degrees, which exceeds the limit.", i, startp[i]);
				return -1;
			}
			if (stopp[i] < m_PosNeg[i] - 0.01 || stopp[i] > m_PosPos[i] + 0.01)
			{
				FX_LOG_ERRO("OnPln: Joint %d's angle is %f degrees, which exceeds the limit.", i, stopp[i]);
				return -1;
			}
		}
	}
