...
int FX_L0_Kinematics_luna_body_init(FX_MotionHandle handle, double DH[6][4], double Flange_length,
                                    double lmt_neg[6], double lmt_pos[6])
{
    if (!handle || !DH || !Flange_length || !lmt_neg || !lmt_pos)
        return FX_MOTION_INVALID_INPUT;

    handle->luna_body_kine.OnSetLunaBodyDH(DH, Flange_length, lmt_neg, lmt_pos);

    FX_LOG_INFO("Set Luna configuration success.");
    return FX_MOTION_OK;
}

int FX_L0_Kinematics_luna_body_forward_kinematics(FX_MotionHandle handle, double jv[6],
                                                  double pgL[4][4], double pgR[4][4])
{
    if (!handle || !jv || !pgL || !pgR)
        return FX_MOTION_INVALID_INPUT;

    Vect6 input_joints = {0};
    for (int i = 0; i < 6; i++)
    {
        input_joints[i] = jv[i];
    }
    handle->luna_body_kine.OnKine6DOF(input_joints, pgL, pgR);

    FX_LOG_INFO("Calculate luna shoulder pose success");
    return FX_MOTION_OK;
}

int FX_L0_Kinematics_luna_body_inverse_kinematics(FX_MotionHandle handle, double pgL[4][4], double pgR[4][4],
                                                  double ref_joints[6], double ret_joints[6])
{
    if (!handle || !pgL || !pgR || !ref_joints || !ret_joints)
        return FX_MOTION_INVALID_INPUT;

    Matrix4 pose_right = {0};
    Matrix4 pose_left = {0};
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            pose_left[i][j] = pgL[i][j];
            pose_right[i][j] = pgR[i][j];
        }
    }

    Vect6 luna_ref_joints = {0};
    for (int i = 0; i < 6; i++)
    {
        luna_ref_joints[i] = ref_joints[i];
    }

    if (!handle->luna_body_kine.OnInvKine6DOF(pose_left, pose_right, luna_ref_joints, ret_joints))
    {
        return FX_MOTION_ERROR;
    }

    FX_LOG_INFO("Calculate inverse kinematics of luna shoulder pose success");
    return FX_MOTION_OK;
}
...
int FX_L0_CPointSet_OnAppendPoint(void *pset, double *point_value)
{
    ...
    FX_LOG_INFO("Please check the length of the input array to avoid values ​​exceeding the bounds.");
