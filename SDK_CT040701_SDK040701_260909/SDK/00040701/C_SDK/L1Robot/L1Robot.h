...
extern "C"
{
     ...
     * @return void
     */
    FX_L1_SDK_API void FX_L1_System_Unlink(void);
    /**
     ...
     */
    FX_L1_SDK_API int FX_L1_System_GetLinkState(void);
    /**
     ...
     */
    FX_L1_SDK_API unsigned int FX_L1_System_GetLogLevel(void);
     ...
     */
    FX_L1_SDK_API int FX_L1_System_GetControllerVersion(void);
     ...
     */
    FX_L1_SDK_API int FX_L1_System_GetSDKVersion(void);
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_System_Reboot(void);
    ...
    /**
     * @brief Restore the system image from a backup.
     *
     * This function performs a full system image restore using the backup
     * image source. It is typically used during factory reset, recovery boot, 
     * or firmware rollback scenarios.
     *
     * @note This operation may erase all current user data and configurations.
     *       Ensure any required data is backed up before invoking this API.
     *
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_System_RestoreImage(void);

    /**
     * @brief Set the user feedback type for a specified channel.
     *
     * This function configures the feedback type reported by the user logic
     * layer on the given channel. The feedback type determines how the system
     * interprets and processes status or measurement data returned from the
     * hardware or lower-level modules.
     *
     * @param[in] chn_id    Channel identifier (0-3).
     * @param[in] fbk_type  Desired feedback type. Must be one of the values
     *                      defined by @ref FXUserFbkType.
     *
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_System_SetUserFbkType(int chn_id, int fbk_type);    

    /**
     * @brief Set the robot serial number.
     *
     * @param[in] serial  Null-terminated string containing the serial number.
     *                    Maximum length is 29 characters (plus null terminator)
     *                    to fit within the 30-byte buffer.
     *
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_System_SetSN(char serial[30]);
    /**
     * @brief Retrieve the robot serial number.
     *
     * @param[out] serial  Buffer to receive the null-terminated serial number string.
     *                     Must be at least 30 bytes in size to accommodate up to
     *                     29 characters plus the null terminator.
     *
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_System_GetSN(char serial[30]);
    /* =========================================================
     * Feedback APIs
     * ========================================================= */
     ...
     * @see FXRobotType
     */
    FX_L1_SDK_API FXRobotType FX_L1_Fbk_GetRobotType(void);
     ...
     */
    FX_L1_SDK_API const ROBOT_RT *FX_L1_Fbk_GetRT(void);
     ...
     */
    FX_L1_SDK_API const ROBOT_SG *FX_L1_Fbk_GetSG(void);
     ...
     */
    FX_L1_SDK_API void FX_L1_Fbk_ResetUserDataSet(void);
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Fbk_ResetSystemMsg(void);
     ...
     * @param obj_type Target object type.
     * @param timeout Maximum time allowed for transition.
     * @param ref_ori_type Reference orientation type.
    ...
    /**
     * @brief Switch the controller state to MIT Mode.
     *
     * Transitions the low-level joint controller into MIT control mode, which implements
     * a high-performance torque-control scheme combining position/velocity feedback with
     * feedforward torque commands. In this mode, the controller computes the output torque as:
     * @f$ \tau = K_p (q_{des} - q) + K_d (\dot{q}_{des} - \dot{q}) + \tau_{ff} @f$.
     *
     * This mode is typically used for compliant robotic applications requiring precise
     * impedance shaping and external force interaction. The transition must satisfy internal
     * safety checks; otherwise, the state switch will fail after the specified timeout.
     *
     * @param[in] obj_type   Target object identifier (e.g., specific robot arm or module).
     * @param[in] timeout    Maximum blocking time (in milliseconds) allowed for the state
     *                      transition to complete. A value of 0 implies non-blocking behavior.
     * @param[in] k          Position gain (stiffness) vector K_p for each of the 7 joints [Nm/°].
     * @param[in] d          Velocity gain (damping) vector K_d for each of the 7 joints [Nm·s/°].
     *
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_State_SwitchToMITMode(FXObjType obj_type, 
                                                    unsigned int timeout, 
                                                    double k[7], 
                                                    double d[7]);

    /**
     * @brief Switch the controller state to user defined MIT Mode.
     *
     * Transitions the low-level joint controller into MIT control mode, which implements
     * a high-performance torque-control scheme combining position/velocity feedback with
     * feedforward torque commands. In this mode, the controller computes the output torque as:
     ...
     * @param[in] timeout    Maximum blocking time (in milliseconds) allowed for the state
     *                      transition to complete. A value of 0 implies non-blocking behavior.
     * @param[in] pos        Desired joint position vector for each of the 7 joints [°].
     * @param[in] vel        Desired joint velocity vector for each of the 7 joints [°/s].
     * @param[in] tor        Feedforward torque vector for each of the 7 joints [Nm].
     * @param[in] k          Position gain (stiffness) vector K_p for each of the 7 joints [Nm/°].
     * @param[in] d          Velocity gain (damping) vector K_d for each of the 7 joints [Nm·s/°].
     *
     * @return Defined in FXFuncReturn.
     ...
     */
    FX_L1_SDK_API int FX_L1_State_SwitchToUserMITMode(FXObjType obj_type, 
                                                        unsigned int timeout, 
                                                        double pos[7], 
                                                        double vel[7], 
                                                        double tor[7], 
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Terminal_ClearData(FXObjType obj_type);
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Terminal_GetData(FXObjType obj_type,
                                             unsigned int timeout,
                                             FXChnType *chn_type,
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Terminal_SetData(FXObjType obj_type,
                                             FXChnType chn_type,
                                             unsigned int timeout,
                                             unsigned char data[64],
    ...
    /* =========================================================
     * Hand Communication APIs
     * ========================================================= */

    /**
     * @brief Receive data from a hand.
     *
     * @param obj_type Target object type.
     * @param hand_type Output hand type defined by FXHandType.
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn     
     */
     FX_L1_SDK_API int FX_L1_Hand_GetType(FXObjType obj_type, int* hand_type);

    /**
     * @brief Receive data from a hand.
     *
     * @param obj_type Target object type.
     * @param serial_ptr Output data serial pointer.
     * @param hand_out_ptr Output data buffer.
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn     
     */
     FX_L1_SDK_API int FX_L1_Hand_GetData(FXObjType obj_type, int* serial_ptr, unsigned char data[512]);
 
    /**
     * @brief Send data to a hand.
     *
     * @param obj_type Target object type.
     * @param data Input data buffer.
     * @return Defined in FXFuncReturn.
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Hand_SetData(FXObjType obj_type, unsigned char data[512]);                                              
    ...
    /**
     * @brief Enable interference check module with specified configuration files
     *
     * Activate collision/interference detection module, load definition files for coordinate system,
     * calibrated link, input parameters, convex collision geometry and inertial characteristic.
     * Interference checking will be performed inside subsequent motion planning and running cycle.
     *
     * @param[in] CordDef Null‑terminated string path of coordinate definition file
     * @param[in] CalLinkDef Null‑terminated string path of calibrated link definition file
     * @param[in] InputDef Null‑terminated string path of interference calculation input definition file
     * @param[in] ConvexDef Null‑terminated string path of convex collision geometry definition file
     * @param[in] ICDef Null‑terminated string path of inertial characteristic definition file
     *
     * @return Defined in FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Config_SetInterferenceEnable(char *CordDef, char *CalLinkDef, char *InputDef, char *ConvexDef, char *ICDef);

    /**
     * @brief Disable interference check module
     *
     * Deactivate collision/interference detection module. Clear loaded interference configuration,
     * no interference judgement will be executed during subsequent motion computation.
     *
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SetInterferenceDisable();

    /**
     * @brief Write an 8-bit signed integer (INT8) to a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     * @param[in] object_subindex Sub-index of the target object (0x00~0xFF).
     * @param[in] object_data    Value to be written
     *
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoWriteInt8(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, int8_t object_data);

    /**
     * @brief Read an 8-bit signed integer (INT8) from a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     * @param[in] object_subindex Sub-index of the target object (0x00~0xFF).
     * @param[in] object_data    Point of value to be read
     *
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoReadInt8(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, int8_t* object_data);
    
    /**
     * @brief Write an 8-bit unsigned integer (UINT8) to a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoWriteUInt8(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, uint8_t object_data);
    
    /**
     * @brief Read an 8-bit unsigned integer (UINT8) from a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoReadUInt8(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, uint8_t* object_data);
    
    /**
     * @brief Write an 16-bit signed integer (INT16) to a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoWriteInt16(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, int16_t object_data);
    
    /**
     * @brief Read an 16-bit signed integer (INT16) from a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoReadInt16(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, int16_t* object_data);
    
    /**
     * @brief Write an 16-bit unsigned integer (UINT16) to a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoWriteUInt16(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, uint16_t object_data);
    
    /**
     * @brief Read an 16-bit unsigned integer (UINT16) from a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoReadUInt16(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, uint16_t* object_data);
    
    /**
     * @brief Write an 32-bit signed integer (INT32) to a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoWriteInt32(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, int32_t object_data);
    
    /**
     * @brief Read an 32-bit signed integer (INT32) from a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoReadInt32(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, int32_t* object_data);
    
    /**
     * @brief Write an 32-bit unsigned integer (UINT32) to a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoWriteUInt32(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, uint32_t object_data);
    
    /**
     * @brief Read an 32-bit unsigned integer (UINT32) from a EtherCAT SDO object.
     *
     * @param[in] master_id      Master (segment) identifier. Valid range: 0~2.
     * @param[in] slave_pos      Slave position on the bus. Valid range: 0~15.
     * @param[in] object_index   Object dictionary index (0x0000~0xFFFF).
     ...
     * @return Defined in FXFuncReturn.
     */
    FX_L1_SDK_API int FX_L1_Config_SdoReadUInt32(int master_id, int slave_pos, unsigned int object_index, unsigned int object_subindex, uint32_t* object_data);
    ...
    /**
     * @brief Update joint control commands in MIT Mode during runtime.
     *
     ...
     *
     * @param[in] thread_id  Identifier of the calling thread; used for multi-thread
     *                      synchronization and priority management within the SDK.
     * @param[in] obj_type   Target object identifier (e.g., robotic arm or actuator group).
     * @param[in] pos        Desired joint position vector for each of the 7 joints [°].
     * @param[in] vel        Desired joint velocity vector for each of the 7 joints [°/s].
     * @param[in] tor        Feedforward torque vector for each of the 7 joints [Nm].
     * @param[in] k          Position gain (stiffness) vector K_p for each of the 7 joints [Nm/°].
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Runtime_SetJointMITCmd(unsigned int thread_id, FXObjType obj_type, double pos_cmd[7], double vel_cmd[7], double tor_cmd[7], double k[7], double d[7]);
    ...
    /**
     * @brief Set the reference orientation for a runtime-controlled object.
     *
     * @param thread_id Thread ID for this function call, range 1~7.
     * @param obj_type Target object type.
     * @param[in] ref_ori_type  Representation type of the reference orientation.
     *                          See ::FXRefOriType for supported formats.
     * @param[in] ref_ori       Array containing the reference orientation data.
     *                          - For Euler angles (RPY): [roll, pitch, yaw] (degree).
     *                          Required length depends on @p ref_ori_type.
     *
     * @see FXFuncReturn
     */

    FX_L1_SDK_API int FX_L1_Runtime_SetRefOri(unsigned int thread_id, FXObjType obj_type, FXRefOriType ref_ori_type, double ref_ori[3]);
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Runtime_SetPDP(unsigned int thread_id, FXObjType obj_type, double p[7]);
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Runtime_SetPDD(unsigned int thread_id, FXObjType obj_type, double d[7]);
     ...
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Runtime_SetPD(unsigned int thread_id, FXObjType obj_type, double p[7], double d[7]);
    ...
    /**
     * @brief Initialize the Luna body kinematics environment using default ini
     *        configuration. Robot must be connected.
     *
     * @param[in] handle Kinematics context handle.
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Kinematics_LunaBodyInit_ByIniConfig(FX_MotionHandle handle);

    /**
     * @brief Initialize the Luna body kinematics environment using input parameters.
     *
     * @param[in] handle Kinematics context handle.
     * @param[in] dh_params Luna body DH parameter table, size `[6][4]`.
     * @param[in] flange_length Luna body flange length.
     * @param[in] joint_limit_neg Lower joint limits, size `[6]`.
     * @param[in] joint_limit_pos Upper joint limits, size `[6]`.
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Kinematics_LunaBodyInit_ByInputParams(FX_MotionHandle handle, double dh_params[6][4], double flange_length,
                                                                 double joint_limit_neg[6], double joint_limit_pos[6]);
    /**
     * @brief Compute the left and right shoulder poses using Luna body forward
     *        kinematics.
     *
     * @param[in] handle Kinematics context handle.
     * @param[in] jv Luna body joint values, length 6.
     * @param[out] arm0_shoulder_matrix Arm 0 shoulder pose represented as a
     *                                  4-by-4 homogeneous transformation matrix.
     * @param[out] arm1_shoulder_matrix Arm 1 shoulder pose represented as a
     *                                  4-by-4 homogeneous transformation matrix.
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Kinematics_LunaBodyForwardKinematics(FX_MotionHandle handle, double jv[6],
                                                                 double arm0_shoulder_matrix[4][4], double arm1_shoulder_matrix[4][4]);

    /**
     * @brief Compute Luna body joint values from the left and right shoulder
     *        poses.
     *
     * @param[in] handle Kinematics context handle.
     * @param[in] arm0_shoulder_matrix Arm 0 shoulder pose represented as a
     *                                 4-by-4 homogeneous transformation matrix.
     * @param[in] arm1_shoulder_matrix Arm 1 shoulder pose represented as a
     *                                 4-by-4 homogeneous transformation matrix.
     * @param[in] luna_body_ref_joints Reference Luna body joint values used to
     *                                 select an inverse kinematics solution,
     *                                 length 6.
     * @param[out] luna_body_result_joints Computed Luna body joint values,
     *                                     length 6.
     * @return Defined in FXFuncReturn.
     *
     * @see FXFuncReturn
     */
    FX_L1_SDK_API int FX_L1_Kinematics_LunaBodyInverseKinematics(FX_MotionHandle handle,
                                                                 double arm0_shoulder_matrix[4][4], double arm1_shoulder_matrix[4][4],
