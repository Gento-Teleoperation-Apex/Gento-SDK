...
void CICBase::FX_OnGetCoM(double mcp_average[3])
{
	...
	{
		FX_PGPointMap(m_Cords[i].m_RunW, m_Cords[i].m_Mcp, m_Cords[i].m_McpW);
		mcp[0] += m_Cords[i].m_McpW[0]*m_Cords[i].m_M;
		mcp[1] += m_Cords[i].m_McpW[1]*m_Cords[i].m_M;
		mcp[2] += m_Cords[i].m_McpW[2]*m_Cords[i].m_M;
		m += m_Cords[i].m_M;
	}
	mcp_average[0] = mcp[0] / m;
	mcp_average[1] = mcp[1] / m;
	mcp_average[2] = mcp[2] / m;
}

void CICBase::FX_OnGetInputNum(long &input_num)
{
	input_num = m_Cord_input_Num;
}

void CICBase::FX_OnGetInterfNum(long &interf_num)
{
	interf_num = m_ICsNum;
}
...
bool CICBase::FX_OnLoadCord(char *CordDef)
{
	...
	for (i = 0; i < cord_num; i++)
	{
		...
		if (cord_cfg.FX_OnGetLong(type) == false)
		{
			cord_cfg.FX_OnClose();
			FX_OnEmtpy();
			return false;
		}
		if (cord_cfg.FX_OnGetVect3(mcp) == false)
		{
			cord_cfg.FX_OnClose();
			FX_OnEmtpy();
			return false;
		}
		if (cord_cfg.FX_OnGetDouble(m) == false)
		{
			cord_cfg.FX_OnClose();
			FX_OnEmtpy();
			return false;
		}
		memcpy(m_Cords[i].m_Mcp, mcp, sizeof(mcp));
		memcpy(m_Cords[i].m_McpW, mcp, sizeof(mcp));
		m_Cords[i].m_Type = type;
		m_Cords[i].m_M = m;
