...
int FX_Interf_CalInterfDistance(FX_InterfHandle handle)
{
    if (!handle->_init_flag)
    {
        return FUNC_RET_INVALID_INPUT_ARG;
...
int FX_Interf_GetInputNum(FX_InterfHandle handle, long *input_num)
{
    if (!handle->_init_flag)
    {
        return FUNC_RET_INVALID_INPUT_ARG;
    ...
    long num = 0;
    handle->_base->FX_OnGetInputNum(num);
    *input_num = num;
    return FUNC_RET_SUCCESS;
}
...
int FX_Interf_GetInterfSpan(FX_InterfHandle handle, long serial, double *MinSpan)
{
    ...
    double minspan = 0;
    if(!handle->_base->FX_OnGetInterfSpan(serial, minspan))
    {
        return FUNC_RET_OPERATION_FAILED;
    }
    *MinSpan = minspan;
...
int FX_Interf_GetInterfNum(FX_InterfHandle handle, long *interf_num)
{
    if (!handle->_init_flag)
    {
        return FUNC_RET_INVALID_INPUT_ARG;
    ...
    long num = 0;
    handle->_base->FX_OnGetInterfNum(num);
    * interf_num = num;
    return FUNC_RET_SUCCESS;
}
