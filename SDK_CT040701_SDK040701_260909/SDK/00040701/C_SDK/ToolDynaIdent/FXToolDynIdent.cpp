...
int Position_Sampling(int arm, char *file_path, int loadcode, TaskContext& task) {
    ...
    double vel_ratio = 100.0;              ///< Velocity ratio for position mode
    double acc_ratio = 100.0;              ///< Acceleration ratio for position mode
...
int FX_ToolDyn_Run_Load_Identification(int RobotSerial,FXFuncReturn* error_code,
                        ...
                        )
{
    ...
    int robot_type= 1;
    int ret = 0;
    char    name_flange_d[30] = ""; 
    char    name_link4_d[30] = "";
    float   flange_d = 0.0;
    float   link4_d = 0.0;

    /* Calculate Load Dynamics parameters */
    snprintf(name_flange_d, sizeof(name_flange_d), "R.A%d.FLANGE.D", RobotSerial);
    snprintf(name_link4_d, sizeof(name_link4_d), "R.A%d.L4.DH.D", RobotSerial);
    if ((FX_L1_Param_GetFloat(name_flange_d, &flange_d) != FUNC_RET_SUCCESS) || (FX_L1_Param_GetFloat(name_link4_d, &link4_d) != FUNC_RET_SUCCESS))
    {
        printf("Failed to get flange_d or link4_d from robot.ini\n");
        return -1;
    }
    IdenCalParaCfg para;
    para.l4 = link4_d;
    ...
    ret = OnCalLoadDyn(&para, out_dynPara, robot_type, UserPath);
    if (ret != 0) {
