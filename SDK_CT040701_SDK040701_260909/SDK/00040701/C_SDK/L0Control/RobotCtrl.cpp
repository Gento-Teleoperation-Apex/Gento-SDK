...
RobotCtrl::RobotCtrl()
{
	...
	m_RobotSGRecvTag = 0;
	m_ParaSerial = 0;
	m_LocalLogTag = FX_FALSE;
	m_LinkState = FX_FALSE;
	m_LinkLifeCount = 0;
...
FX_BOOL RobotCtrl::Link(FX_UCHAR ip1, FX_UCHAR ip2, FX_UCHAR ip3, FX_UCHAR ip4)
{
	...
	if (ins->m_Hand0_NA.OnLinkTo(ip1, ip2, ip3, ip4, 3726) == FX_FALSE)
	{
		ins->m_RT_NA.OnUnlink();
		ins->m_SG_NA.OnUnlink();
		ins->m_Flange_NA0.OnUnlink();
		ins->m_Flange_NA1.OnUnlink();
		ins->m_Msg_NA.OnUnlink();
        ins->m_Hand0_NA.OnUnlink();
		return FX_FALSE;
	}    
	if (ins->m_Hand1_NA.OnLinkTo(ip1, ip2, ip3, ip4, 3727) == FX_FALSE)
	{
		ins->m_RT_NA.OnUnlink();
		ins->m_SG_NA.OnUnlink();
		ins->m_Flange_NA0.OnUnlink();
...
#ifdef CMPL_LIN
	{
		...
		ret = timer_create(CLOCK_MONOTONIC, &evp, &ins->robot_timer);
		if (ret)
		{
			ins->m_RT_NA.OnUnlink();
			ins->m_SG_NA.OnUnlink();
		...
		ret = timer_settime(ins->robot_timer, 0, &ts, NULL);
		if (ret)
		{
			ins->m_RT_NA.OnUnlink();
			ins->m_SG_NA.OnUnlink();
...
#ifdef CMPL_LIN
	if (ins->robot_timer != NULL)
	{
		timer_delete(ins->robot_timer);
		ins->robot_timer = NULL;
	}
#endif

	CUtility::UniMilliSleep(3);
...
FX_INT32 RobotCtrl::TestLink()
{
    ...
    ins->m_Hand0_NA.OnSendLink();
    ins->m_Hand1_NA.OnSendLink();
...
FX_INT32 RobotCtrl::System_GetControllerVersion()
{
	...
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return -2;
	}
...
FX_BOOL RobotCtrl::System_Reboot()
{
	...
	return SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf);
}
...
FX_BOOL RobotCtrl::System_Update()
{
    ...
    return ins->WaitOpReturn(serial, 1000);
}
...
FX_BOOL RobotCtrl::System_RestoreImage()
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_RESTORE_IMAGE;
...
FX_BOOL RobotCtrl::System_SetUserFbkType(FX_INT32 chn_id, FX_INT16 fbk_type)
{
    ...
    switch(chn_id)
    {
        case 0: return RobotCtrl::SetShortInt(FX_DEFAULT_THREAD_ID, UDP_SYSTEM_RT_UserFbkType0, 1, &fbk_type);
        case 1: return RobotCtrl::SetShortInt(FX_DEFAULT_THREAD_ID, UDP_SYSTEM_RT_UserFbkType1, 1, &fbk_type);
        case 2: return RobotCtrl::SetShortInt(FX_DEFAULT_THREAD_ID, UDP_SYSTEM_RT_UserFbkType2, 1, &fbk_type);
        case 3: return RobotCtrl::SetShortInt(FX_DEFAULT_THREAD_ID, UDP_SYSTEM_RT_UserFbkType3, 1, &fbk_type);
        default: return FX_FALSE;
    }
}

FX_BOOL RobotCtrl::System_SetSN(char serial_number[30])
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_SET_SN;
    strncpy(buf.m_OpValueS, serial_number, sizeof(buf.m_OpValueS) - 1);
    buf.m_OpValueS[sizeof(buf.m_OpValueS) - 1] = '\0';
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
	}
...
FX_BOOL RobotCtrl::System_GetSN(char serial_number[30])
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_GET_SN;
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
	}
	if (!ins->WaitOpReturn(serial, 1000))
	{
		return FX_FALSE;
	}
	strncpy(serial_number, ins->m_RobotSG.m_OP_SET.m_OpValueS, 29);
	serial_number[29] = 0;
	return FX_TRUE;
}

FX_BOOL RobotCtrl::System_SdoWriteInt8(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_INT8 object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();

    if(master_id < 0 || master_id > 2
        || slave_pos < 0 || slave_pos > 15)
    {
        return FX_FALSE;
    }
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_SET_SDO_INT8;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
    memcpy(buf.m_OpValueS, &object_data, 1);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
	}
	return ins->WaitOpReturn(serial, 10000); // 10 seconds
}

FX_BOOL RobotCtrl::System_SdoReadInt8(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_INT8* object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_GET_SDO_INT8;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
	}
	if (!ins->WaitOpReturn(serial, 10000)) // 10 seconds
	{
		return FX_FALSE;
	}
    memcpy(object_data, ins->m_RobotSG.m_OP_SET.m_OpValueS, 1);
    return FX_TRUE;
}

FX_BOOL RobotCtrl::System_SdoWriteUInt8(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_UINT8 object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_SET_SDO_UINT8;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
    memcpy(buf.m_OpValueS, &object_data, 1);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
...
FX_BOOL RobotCtrl::System_SdoReadUInt8(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_UINT8* object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_GET_SDO_UINT8;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
...
FX_BOOL RobotCtrl::System_SdoWriteInt16(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_INT16 object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_SET_SDO_INT16;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
    memcpy(buf.m_OpValueS, &object_data, 2);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
	}
...
FX_BOOL RobotCtrl::System_SdoReadInt16(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_INT16* object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_GET_SDO_INT16;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
    ...
    memcpy(object_data, ins->m_RobotSG.m_OP_SET.m_OpValueS, 2);
    return FX_TRUE;
}

FX_BOOL RobotCtrl::System_SdoWriteUInt16(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_UINT16 object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_SET_SDO_UINT16;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
    memcpy(buf.m_OpValueS, &object_data, 2);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
...
FX_BOOL RobotCtrl::System_SdoReadUInt16(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_UINT16* object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_GET_SDO_UINT16;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
...
FX_BOOL RobotCtrl::System_SdoWriteInt32(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_INT32 object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_SET_SDO_INT32;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
    memcpy(buf.m_OpValueS, &object_data, 4);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
	}
...
FX_BOOL RobotCtrl::System_SdoReadInt32(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_INT32* object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_GET_SDO_INT32;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
    ...
    memcpy(object_data, ins->m_RobotSG.m_OP_SET.m_OpValueS, 4);
    return FX_TRUE;
}

FX_BOOL RobotCtrl::System_SdoWriteUInt32(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_UINT32 object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_SET_SDO_UINT32;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
    memcpy(buf.m_OpValueS, &object_data, 4);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
...
FX_BOOL RobotCtrl::System_SdoReadUInt32(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex, FX_UINT32* object_data)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_SYSTEM_GET_SDO_UINT32;
    buf.m_OpValueI = PackSdoInsIndex(master_id, slave_pos, object_index, object_subindex);
	if (!SetRawData(FX_DEFAULT_THREAD_ID, UDP_OPERATION, sizeof(OP_SET), (FX_UCHAR *)&buf))
	{
		return FX_FALSE;
...
FX_BOOL RobotCtrl::Para_GetInt(FX_CHAR name[30], FX_INT32 *ret_value)
{
	...
	strncpy(buf.m_OpValueS, name, sizeof(buf.m_OpValueS) - 1);
	buf.m_OpValueS[sizeof(buf.m_OpValueS) - 1] = '\0';
...
FX_BOOL RobotCtrl::Arm1_Terminal_SetData(FX_INT32 channel_type, FX_UCHAR *data_ptr, FX_INT32 data_len)
{
	...
	FX_INT32 serial = ins->pDDSS2->m_Serial + 1;
	if (serial > 1000000)
	{
		serial = 1;
	}
...
FX_BOOL RobotCtrl::Arm0_Hand_GetType(FX_INT32* hand_type)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_ARM0_HAND_GET_TYPE;
	...
	*hand_type = ins->m_RobotSG.m_OP_SET.m_OpValueI;
	return FX_TRUE;
}

FX_BOOL RobotCtrl::Arm0_Hand_GetData(FX_INT32* data_serial_ptr, FX_UCHAR data_ptr[512])
{
    if(data_serial_ptr == NULL || data_ptr == NULL)
    {
        return FX_FALSE;
    }
    
    RobotCtrl *ins = RobotCtrl::GetIns();
    *data_serial_ptr = ins->m_HandOUT[0].m_FrameSerial;
    memcpy(data_ptr, ins->m_HandOUT[0].data, 512);
    return FX_TRUE;
}

FX_BOOL RobotCtrl::Arm0_Hand_SetData(FX_UCHAR data_ptr[512])
{
    if(data_ptr == NULL)
    {
        return FX_FALSE;
    ...
    RobotCtrl *ins = RobotCtrl::GetIns();
    ins->m_Hand0_NA.m_buf.m_SendBuf[0] = 'F';
    ins->m_Hand0_NA.m_buf.m_SendBuf[1] = 'C';
	memcpy(&ins->m_Hand0_NA.m_buf.m_SendBuf[2], data_ptr, 512);
	ins->m_Hand0_NA.m_buf.m_Slen = 514;
	ins->m_Hand0_NA.m_buf.m_STag = 100;
	return ins->m_Hand0_NA.OnSendRaw();
}

FX_BOOL RobotCtrl::Arm1_Hand_GetType(FX_INT32* hand_type)
{
	RobotCtrl *ins = RobotCtrl::GetIns();
	...
	memset(&buf, 0, sizeof(OP_SET));
	buf.m_OpCmdSerial = serial;
	buf.m_OpIns = OPINS_ARM1_HAND_GET_TYPE;
...
FX_BOOL RobotCtrl::Arm1_Hand_GetData(FX_INT32* data_serial_ptr, FX_UCHAR data_ptr[512])
{
    if(data_serial_ptr == NULL || data_ptr == NULL)
    {
        return FX_FALSE;
    ...
    RobotCtrl *ins = RobotCtrl::GetIns();
    *data_serial_ptr = ins->m_HandOUT[1].m_FrameSerial;
    memcpy(data_ptr, ins->m_HandOUT[1].data, 512);
    return FX_TRUE;
}

FX_BOOL RobotCtrl::Arm1_Hand_SetData(FX_UCHAR data_ptr[512])
{
    if(data_ptr == NULL)
    {
        return FX_FALSE;
    ...
    RobotCtrl *ins = RobotCtrl::GetIns();
    ins->m_Hand1_NA.m_buf.m_SendBuf[0] = 'F';
    ins->m_Hand1_NA.m_buf.m_SendBuf[1] = 'C';
	memcpy(&ins->m_Hand1_NA.m_buf.m_SendBuf[2], data_ptr, 512);
	ins->m_Hand1_NA.m_buf.m_Slen = 514;
	ins->m_Hand1_NA.m_buf.m_STag = 100;
	return ins->m_Hand1_NA.OnSendRaw();
}
...
FX_BOOL RobotCtrl::Arm0_Runtime_SetPDP(FX_UINT32 thread_id, FX_DOUBLE p[7])
{
	return RobotCtrl::SetFLoat(thread_id, UDP_ARM0_SG_PDK, 7, p);
}

FX_BOOL RobotCtrl::Arm0_Runtime_SetPDD(FX_UINT32 thread_id, FX_DOUBLE d[7])
{
	return RobotCtrl::SetFLoat(thread_id, UDP_ARM0_SG_PDD, 7, d);
}
...
FX_BOOL RobotCtrl::Arm0_Runtime_SetRefOri(FX_UINT32 thread_id, FX_INT16 ref_ori_type, FX_DOUBLE ref_ori[3])
{
    if(ref_ori_type < 0 || ref_ori_type > 2)
    {
        return FX_FALSE;
    }
    FX_UCHAR data[14] = {0};
    FX_UINT16 ori_type = ref_ori_type;
    memcpy(&data[0], &ori_type, sizeof(FX_UINT16));
    if(ref_ori != NULL)
    {
        FX_FLOAT ori[3];
        ori[0] = ref_ori[0];
        ori[1] = ref_ori[1];
        ori[2] = ref_ori[2];
        memcpy(&data[2], ori, sizeof(FX_FLOAT) * 3);
    }
    return RobotCtrl::SetRawData(thread_id, UDP_ARM0_RT_RefOri, 14, data);
}
...
FX_BOOL RobotCtrl::Arm0_Runtime_SetJointMITCmd(FX_UINT32 thread_id, FX_DOUBLE pos_cmd[7], FX_DOUBLE vel_cmd[7], FX_DOUBLE tor_cmd[7], FX_DOUBLE k[7], FX_DOUBLE d[7])
{
	FX_FLOAT data[35];
	...
	return RobotCtrl::SetRawData(thread_id, UDP_ARM0_SP_CmdMIT, sizeof(FX_FLOAT) * 35, (FX_UCHAR *)data);
}
...
FX_BOOL RobotCtrl::Arm0_Runtime_SetTraj(FX_UINT32 thread_id, FX_INT32 serial, FX_INT32 point_num, FX_DOUBLE *point_data)
{
	...
	FX_UCHAR tmp[1450];
	memcpy(&tmp[0], &serial, sizeof(FX_INT32));
	FX_INT32 spos = 0;
	for (FX_INT32 i = 0; i < point_num; i++)
	{
		for (FX_INT32 j = 0; j < 7; j++)
		{
			FX_FLOAT v = point_data[spos];
			memcpy(&tmp[sizeof(FX_INT32) + spos * sizeof(FX_FLOAT)], &v, sizeof(FX_FLOAT));
			spos++;
		}
	}
...
FX_BOOL RobotCtrl::Arm1_Runtime_SetPDP(FX_UINT32 thread_id, FX_DOUBLE p[7])
{
	return RobotCtrl::SetFLoat(thread_id, UDP_ARM1_SG_PDK, 7, p);
}

FX_BOOL RobotCtrl::Arm1_Runtime_SetPDD(FX_UINT32 thread_id, FX_DOUBLE d[7])
{
	return RobotCtrl::SetFLoat(thread_id, UDP_ARM1_SG_PDD, 7, d);
}
...
FX_BOOL RobotCtrl::Arm1_Runtime_SetRefOri(FX_UINT32 thread_id, FX_INT16 ref_ori_type, FX_DOUBLE ref_ori[3])
{
    if(ref_ori_type < 0 || ref_ori_type > 2)
    {
        return FX_FALSE;
    ...
    return RobotCtrl::SetRawData(thread_id, UDP_ARM1_RT_RefOri, 14, data);
}
...
FX_BOOL RobotCtrl::Arm1_Runtime_SetJointMITCmd(FX_UINT32 thread_id, FX_DOUBLE pos_cmd[7], FX_DOUBLE vel_cmd[7], FX_DOUBLE tor_cmd[7], FX_DOUBLE k[7], FX_DOUBLE d[7])
{
	FX_FLOAT data[35];
	for (FX_INT32 i = 0; i < 7; i++)
	{
	...
	return RobotCtrl::SetRawData(thread_id, UDP_ARM1_SP_CmdMIT, sizeof(FX_FLOAT) * 35, (FX_UCHAR *)data);
}
...
RobotCtrl *RobotCtrl::GetIns()
{
	...
	std::call_once(once, []() { m_InsRobot = new RobotCtrl(); });
	return m_InsRobot;
}
...
FX_VOID RobotCtrl::DoRecv()
{
    ...
    static FX_INT32 hand_out_size = sizeof(HAND_OUT);
	...
	if (ins->m_SG_NA.m_buf.m_Rlen > 0)
	{
		if (ins->m_SG_NA.m_buf.m_Rlen == robot_sg_size + 2)
		{
			...
			{
				std::lock_guard<std::mutex> lk(ins->m_OpMutex);
	...
	ins->m_Hand0_NA.OnRecv();
	while (ins->m_Hand0_NA.m_buf.m_Rlen > 0)
	{
		if (ins->m_Hand0_NA.m_buf.m_Rlen == hand_out_size + 2)
		{
		    if(ins->m_Hand0_NA.m_buf.m_Recvbuf[0] == 'F'
                && ins->m_Hand0_NA.m_buf.m_Recvbuf[1] == 'D')
			{
			    memcpy(&ins->m_HandOUT[0], &ins->m_Hand0_NA.m_buf.m_Recvbuf[2], hand_out_size);
            }
			ins->m_Hand0_NA.m_buf.m_Rlen = 0;
		}
		ins->m_Hand0_NA.OnRecv();
	}
	ins->m_Hand1_NA.OnRecv();
	while (ins->m_Hand1_NA.m_buf.m_Rlen > 0)
	{  
		if (ins->m_Hand1_NA.m_buf.m_Rlen == hand_out_size + 2)
		{
		    if(ins->m_Hand1_NA.m_buf.m_Recvbuf[0] == 'F'
                && ins->m_Hand1_NA.m_buf.m_Recvbuf[1] == 'D')
			{
			    memcpy(&ins->m_HandOUT[1], &ins->m_Hand1_NA.m_buf.m_Recvbuf[2], hand_out_size);
            }
			ins->m_Hand1_NA.m_buf.m_Rlen = 0;
		}
		ins->m_Hand1_NA.OnRecv();
	}
	ins->m_Msg_NA.OnRecv();
...
FX_VOID RobotCtrl::DoSend()
{
	...
	if (ins->m_RtSendLock.exchange(FX_TRUE) != FX_FALSE)
	{
		return;
	}
...
FX_BOOL RobotCtrl::WaitOpReturn(FX_INT32 serial, FX_INT32 timeout)
{
	...
	{
		std::lock_guard<std::mutex> lk(ins->m_OpMutex);
		ins->m_RobotSG.m_OP_SET.m_OpRetSerial = 0;
	}
	for (FX_INT32 i = 0; i < timeout; i++)
	{
		...
		{
			std::lock_guard<std::mutex> lk(ins->m_OpMutex);
			ret_s = ins->m_RobotSG.m_OP_SET.m_OpRetSerial;
		}
		if (ret_s != 0 && ret_s % 100 == serial)
		{
...
FX_BOOL RobotCtrl::SetState(FX_UINT32 thread_id, FX_INT32 cmd, FX_INT32 cmd_state)
{
	...
	memcpy(&buf[3], &cmd_state, sizeof(FX_INT16));
...
FX_BOOL RobotCtrl::SetFLoat(FX_UINT32 thread_id, FX_INT32 cmd, FX_INT32 num, FX_DOUBLE *pdata)
{
	...
	{
		FX_FLOAT v = pdata[i];
		memcpy(&buf[3 + i * sizeof(FX_FLOAT)], &v, sizeof(FX_FLOAT));
	}
	for (FX_INT32 i = 0; i < crc_len; i++)
	{
		crc += buf[i];
...
FX_INT32 RobotCtrl::PackSdoInsIndex(FX_INT32 master_id, FX_INT32 slave_pos, FX_UINT16 object_index, FX_UINT8 object_subindex)
{
    FX_INT32 sdo_index = 0;
    sdo_index |= (static_cast<FX_INT32>(object_index & 0xFFFF) << 16);
    sdo_index |= (static_cast<FX_INT32>(object_subindex & 0xFF) << 8);
    sdo_index |= (static_cast<FX_INT32>(master_id & 0x0F) << 4);
    sdo_index |= (static_cast<FX_INT32>(slave_pos & 0x0F) << 0);
    return sdo_index;
}
