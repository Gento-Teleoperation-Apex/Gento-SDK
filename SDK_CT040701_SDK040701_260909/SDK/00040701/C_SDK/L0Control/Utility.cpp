...
FX_BOOL CMarvNetAgent::OnLinkTo(FX_UCHAR ip1, FX_UCHAR ip2, FX_UCHAR ip3, FX_UCHAR ip4, FX_INT32L port)
{
	...
	if (m_nc._local_sock == INVALID_SOCKET)
	{
		m_nc._local_sock = 0;
		return FX_FALSE;
	}
	...
	if (m_nc._tosock_ == INVALID_SOCKET)
	{
		m_nc._tosock_ = 0;
#ifdef CMPL_WIN
		closesocket(m_nc._local_sock);
...
{
	return item_num_.load(std::memory_order_acquire);
}
...
FX_BOOL CACB::Empty()
{
	...
	if (read_lock_.exchange(1) != 0 || write_lock_.exchange(1) != 0)
	{
		read_lock_ = 0;
		return FX_FALSE;
	}
