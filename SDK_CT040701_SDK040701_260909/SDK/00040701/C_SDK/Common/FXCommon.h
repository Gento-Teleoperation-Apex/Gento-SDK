...
#define FX_SDK_MINOR_VERSION 7UL /**< Minor version number */
...
typedef enum
{
    ...
    ARM_STATE_MIT = 4,                 ///< MIT mode
    ARM_STATE_USER_MIT = 5,            ///< User defined MIT mode
    ...
    ARM_STATE_TRANS_TO_MIT = 104,      ///< Transition to MIT mode
    ARM_STATE_TRANS_TO_USER_MIT = 105, ///< Transition to user defined MIT mode
...
typedef enum
{
    ...
    FX_STATE_MIT = 12,           ///< MIT control mode
    FX_STATE_USER_MIT = 13,      ///< User defined MIT control mode
...
/**
 * @brief Reference orientation type.
 */
typedef enum
{
    FX_REFORI_TYPE_NULL = 0,  ///< Reference orientation type disabled
    FX_REFORI_TYPE_ANYBASE = 1, ///< in user defined orientation
    FX_REFORI_TYPE_TCP = 2, ///< in TCP orientation
} FXRefOriType;

/**
 * @brief Source type of user defined feedback channel.
 */
typedef enum
{
    USER_FBK_NULL = 0,
...
typedef struct
{
    FX_FLOAT m_DEVICE_FBK_Robot_IMU[9];  ///< RObot INU feedbacK: ACCX, ACCY,ACCZ,ONGX, OMGY, OMGZ, ANGX, ANGY, ANGZ
    FX_FLOAT m_DEVICE_FBK_Agv_IMU[9];    ///< Agv IMU feedback: ACCX, ACCY,ACCZ,OMGX,OMGY,OMGZ, ANGX, ANGY, ANGZX 
    FX_INT8 m_DEVICE_FBK_Pdo_State[3];  ///< Pdo state of master0/1/2, 0: no pdo; 1: incomplete pdo; 2: complete pdo
    FX_INT8 m_DEVICE_pad[1];
} DEVICE_RT;
...
typedef struct
{
    ...
    FX_INT8 m_ARM_pad[2];
    FX_INT16 m_ARM_CMD_Ctrl_RefOriType;   ///< Reference orientation type
    FX_FLOAT m_ARM_CMD_Ctrl_RefOri[3];    ///< Reference orientation
...
typedef struct
{
    ...
    FX_FLOAT m_ARM_FBK_Joint_GravityTor[7];     ///< Joint gravity compensation torque
...
typedef struct
{
    ...
    FX_FLOAT m_ARM_Ctrl_PDK[7]; 	  ///< Proportional gain of PD
	FX_FLOAT m_ARM_Ctrl_PDD[7]; 	  ///< Derivative gain of PD
    FX_FLOAT m_ARM_Ctrl_ToolKine[6];  ///< Tool kinematics
    FX_FLOAT m_ARM_Ctrl_ToolDyna[10]; ///< Tool dynamics
    FX_BOOL m_ARM_SET_SetTag[20];     ///< Set flags
    FX_BOOL m_ARM_SET_UpdateTag[20];  ///< Update flags
} ARM_SET;
...
typedef struct
{
    ...
    FX_FLOAT m_BODY_FBK_Joint_GravityTor[6];     ///< Joint gravity compensation torque
} BODY_OUT;
...
typedef struct
{
    ...
    FX_INT8 m_BODY_FBK_LowSpdFlag;       ///< Low speed indicator
...
static_assert(sizeof(OP_SET) == 44, "OP_SET layout changed — update hardcoded protocol size and serialization");
...
typedef struct
{
    FX_INT16 m_SYSTEM_UserFbk_Type[4]; 
    FX_FLOAT m_SYSTEM_UserFbk0[8];
    FX_FLOAT m_SYSTEM_UserFbk1[8];
    FX_FLOAT m_SYSTEM_UserFbk2[8];
    FX_FLOAT m_SYSTEM_UserFbk3[8];
}SYSTEM_RT;
...
/**
 * @brief Tool identification process structure.
 */
typedef enum {
    TASK_IDLE = 0,
    TASK_SAMPLING,
